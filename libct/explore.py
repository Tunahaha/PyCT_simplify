...
class ExplorationEngine:
    ...
    def _execution_loop(self, max_iterations, all_args, concolic_dict):
        ...
        iteration_count = 0
        constraint_count = 0
        constraint_size_count=0
        constraint_time_count=0
        total_count=0
        while cont and (max_iterations==0 or iterations<max_iterations):
            ...
            while len(self.constraints_to_solve) > 0:
                ...
                constraint_time_count+=elapsed_time
                file_path = self.save_dir+'constraint_time.txt'
                if not os.path.isfile(file_path):
                    with open(file_path, 'w') as f:
                        f.write(f"Iteration {iteration_count}, Constraint {constraint_count}: Time taken to solve the constraint: {elapsed_time} seconds\n")
                else:
                ...
                if model is not None and not self.only_first_forward:
                    ...
                    if all_args not in tried_input_args:
                        ...
                        break
                # memory_usage = os.popen('"ps -v -p %d" % os.getpid()').readlines()
                # mem = ' '.join(memory_usage[1].split()).split()[7]
                constraint_length = sum(len(str(constraint)) for constraint in self.constraints_to_solve)
                with open('out/cnn/sat_constraint_memory_usage.txt', 'a') as f:
                    f.write(f'SAT constraint size: {len(self.constraints_to_solve)}, Constraint length: {constraint_length}\n')
                file_path = self.save_dir+'sat_constraint_memory_usage.txt'
                if not os.path.isfile(file_path):
                    with open(file_path, 'w') as f:
                        f.write(f"Iteration {iteration_count}, Constraint {constraint_count}: Time taken to solve the constraint: {elapsed_time} seconds\n")
                else:
                    with open(file_path, 'a') as f:
                        f.write(f'SAT constraint size: {len(self.constraints_to_solve)}, Constraint length: {constraint_length}\n')
                constraint_size_mb = sys.getsizeof(str(constraint)) / (1024 * 1024)
                constraint_size_count+=constraint_size_mb
                file_path = self.save_dir+'constraint_size.txt'
                if not os.path.isfile(file_path):
                    with open(file_path, 'w') as f:
                        f.write(f"Iteration {iteration_count}, Constraint {constraint_count}: Size of the constraint: {constraint_size_mb} MB\n")
                else:
                    with open(file_path, 'a') as f:
                        f.write(f"Iteration {iteration_count}, Constraint {constraint_count}: Size of the constraint: {constraint_size_mb} MB\n")
                end_time = time.time()
            ...
            if len(self.constraints_to_solve) == 0:
                ...
                break
        file_path = self.save_dir+'time_size.txt'
        with open(file_path, 'a') as f:
                f.write(f"total size : {constraint_size_count} MB, total time : {constraint_time_count}:Average size: {constraint_size_count/total_count} MB Average time: {constraint_time_count/total_count} seconds\n")
    ...
    def explore(
        ...
        try:
            ...
            file_path = self.save_dir+'iteration_count_timeout.txt'       
            with open(file_path, 'a') as file:
                    file.write(f'Total iterations before timeout: {recorder.total_iter}\n')
        ...
        'Total no timeouts': [total_no_timeouts]
        })
        df.to_csv(self.save_dir+ '/timeouts.csv', index=True)
    ...
    def _one_execution(self, all_args, concolic_dict):
        ...
        if self.previous_result != None and self.previous_result != result:
            print('#'*60)
            print(f'[Result Change]: self.previous_result : {self.previous_result}!= result : {result}')
            print('#'*60)
